...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll('[data-mask-phone]').forEach(input => {
        ...
        input.addEventListener('focus', () => maskPhoneFocus({
            input,
            mask,
            cursorStart,
            cursorEnd,
        ...
        input.addEventListener('blur', () => maskPhoneBlur({
            input,
            mask,
            cursorStart,
            cursorEnd,
...
function wrapController(func) {
    ...
    return function (options) {
        ...
        if (wasEnterNum || wasDel) { // было удаление или введена цифра
            full = (numInput > numMask) ? true : false;
        ...
        // input.value.length > mask.length
        //if ((input.value.length > mask.length) && (cursorInpt - cursorEnd == 1)) {
        if (full && (cursorInpt - cursorEnd == 1)) {
...
function getValueInput(options, newNumber) {
    ...
    console.log(`nextValOne-${nextValOne}, nextValTwo-${nextValTwo}, newNumber-${newNumber}, nextVal-${nextVal}, add-${add}`);
...
function setCursor(options, newNumber) {
    ...
    if (newNumber.length > 0) {
        ...
        if ((cursorEnd - cursorInpt) == 1) { // удаление клавишей BackSpace
...
function madeNewValue(options, getValue) {
    ...
    return (cloneInput) ? val : mask_value
...
// 		owldValue = func.js(options, newNumber);
