...
document.addEventListener("DOMContentLoaded", () => {
	...
	document.querySelectorAll('[data-mask-phone]').forEach(input => {
		...
		input.addEventListener('select', (e) => {
			console.log(e);
			console.log(`input.selectionStart - ${input.selectionStart} \ End - ${input.selectionEnd}`);
...
function maskPhoneInput(options) {
	...
	let { input, mask, event } = options;

	console.log(event);
	console.log(`inputStart - ${input.selectionStart} \ End - ${input.selectionEnd}`);
	...
	let { getValue, cursorSelection } = getValueInput({ input, mask, event }, maskPhoneInput.starting);
